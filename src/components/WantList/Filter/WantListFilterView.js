...
import styles from "./WantListFilterStyles";

const WantListFilterView = ({
  cardStatuses,
  grades,
  handleFilterChanged,
  ...
  <Grid container spacing={2} sx={styles.container}>
    ...
    {!!(Array.isArray(buyOrTrade) && buyOrTrade.length > 0) && (
      <Grid item xs={6}>
        <Typography sx={styles.label} variant="body2">
          Acquisition
        </Typography>
        <MultiSelect
          currentValue={selectedBuyOrTrade}
          onChange={e => handleFilterChanged("buyOrTrade", e.target.value)}
          onDelete={(e, value) =>
            handleFilterChanged(
              "buyOrTrade",
              selectedBuyOrTrade.filter(n => n !== value)
            )
          }
          values={buyOrTrade}
          sx={styles.select}
...
export default WantListFilterView;
